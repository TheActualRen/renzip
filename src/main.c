...
int main() {
	FILE* in = fopen("../test/input.txt", "rb");
	...
	if(blocktype1_encoding(in, out) != 0) {
		fprintf(stderr, "Error: failed to complete BLOCK TYPE 1 encoding\n");
		fclose(in);
		fclose(out);
		exit(1);
	}
	...
	printf("Succesful BLOCK TYPE 1 encoding\n");
